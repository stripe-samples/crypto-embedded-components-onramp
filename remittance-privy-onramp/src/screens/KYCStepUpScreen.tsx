 ...
 * pending status, and polls getOnrampCustomer() until the review resolves.
 ...
 * createRemittance(). Stripe's identity review is asynchronous — always
...
export default function KYCStepUpScreen({ navigation, route }: Props) {
  const {
    ...
    walletAddress, network, sourceAmount,
  ...
  const handleSubmit = async () => {
    ...
    if (needsSsnDob) {
      ...
      dateOfBirth = parseDateOfBirth(dobMonth, dobDay, dobYear);
      if (!dateOfBirth) {
        Alert.alert('Invalid Date of Birth', 'Please enter a valid date of birth in MM/DD/YYYY format.');
        return;
      }
    }
