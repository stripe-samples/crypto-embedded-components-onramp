...
export type StripeApiData = Record<string, unknown> & {
  id?: string;
  status?: string;
  error?: StripeApiError;
};

type OAuthTokenResponse = StripeApiData & {
  access_token?: string;
  refresh?: {
...
function parseStripeData(value: unknown): StripeApiData {
...
export async function stripeCallWithRefresh(
  ...
  record: UserRecord,
  method: string = 'POST',
): Promise<{ response: Response; data: StripeApiData }> {
  ...
  let data = parseStripeData(await response.json());
...
export function toUserError(data: unknown): string {
  if (isRecord(data)) {
...
function isRetryableError(data: StripeApiData): boolean {
